...
// render the table from the data of array
function renderTable() {
    ...
    arr.forEach(el => {
        ...
        <td>
            <div class="name" >
            <img src="${el.img_src}" > ${el.first_name + " " + el.last_name}
            </div>
        </td>
        <td>${el.gender}</td>
        <td>${el.class}</td>
        <td>${el.marks}</td>
        <td>${el.passing ? "Passed" : "Failed"}</td>
        <td>${el.email}</td>
        `
...
// render data when page loads for the first time
renderTable()

// sort array A to Z
function sortAtoZ() {
    arr.sort((a, b) => (a.first_name + a.last_name).localeCompare(b.first_name + b.last_name))
    renderTable()
}

// sort array Z to A
function sortZtoA() {
    arr.sort((a, b) => (b.first_name + b.last_name).localeCompare(a.first_name + a.last_name))
    renderTable()
}
...
function sortByPassing() {
    arr.sort((a, b) => {
        if (a.passing === true && b.passing === false) {
            return -1; 
        } else if (a.passing === false && b.passing === true) {
            return 1; 
        } else {
            return 0; 
        }
...
function handleChange(){
    ...
    arr = arr.filter(el=>(el.first_name+el.last_name).toLowerCase().includes(searchText.toLowerCase().replace(" ",''))||el.email.toLowerCase().includes(searchText.toLowerCase()))
    renderTable()
}
